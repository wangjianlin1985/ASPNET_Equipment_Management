...
namespace DAL
{
    ...
    public class dalUserInfo
    {
        ...
        {
            string sql = "insert into UserInfo(user_name,password,classObj,name,gender,birthDate,userPhoto,telephone,email,address,regTime) values(@user_name,@password,@classObj,@name,@gender,@birthDate,@userPhoto,@telephone,@email,@address,@regTime)";
            /*构建sql参数*/
            ...
            parm[6].Value = userInfo.userPhoto; //用户照片
            parm[7].Value = userInfo.telephone; //联系电话
            parm[8].Value = userInfo.email; //邮箱
        ...
        {
            string sql = "update UserInfo set password=@password,classObj=@classObj,name=@name,gender=@gender,birthDate=@birthDate,userPhoto=@userPhoto,telephone=@telephone,email=@email,address=@address,regTime=@regTime where user_name=@user_name";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@password",SqlDbType.VarChar),
             new SqlParameter("@classObj",SqlDbType.VarChar),
