...
namespace DAL
{
    ...
    public class dalClassInfo
    {
        ...
        {
            string sql = "insert into ClassInfo(classNo,className,bornDate,mainTeacher) values(@classNo,@className,@bornDate,@mainTeacher)";
            /*构建sql参数*/
        ...
        {
            string sql = "update ClassInfo set className=@className,bornDate=@bornDate,mainTeacher=@mainTeacher where classNo=@classNo";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@className",SqlDbType.VarChar),
             new SqlParameter("@bornDate",SqlDbType.DateTime),
            ...
            parm[3].Value = classInfo.classNo;
            /*执行更新*/
            return (DBHelp.ExecuteNonQuery(sql, parm) > 0) ? true : false;
        }
